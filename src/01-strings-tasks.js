...
function removeFirstOccurrences(str, value) {
  const valueFirstOccurrenceIndex = str.indexOf(value);
  const finalStr = str.slice(0, valueFirstOccurrenceIndex)
                   + str.slice(valueFirstOccurrenceIndex + value.length);
  return finalStr;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < strArr.length; i += 1) {
    ...
    if (charCode > 64 && charCode < 123) {
      ...
      if (charCode <= 122 && newCharCode > 122) {
